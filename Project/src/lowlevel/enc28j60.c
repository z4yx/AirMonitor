...
unsigned char enc28j60ReadOp(unsigned char op, unsigned char address)
{
    ...
    ENC28J60_BYTE_RW(dat);
    dat = ENC28J60_BYTE_RW(0xFF);
...
void enc28j60ReadBuffer(unsigned int len, unsigned char* data)
{
    ENC28J60_CSL();
    // issue read command
    ENC28J60_BYTE_RW(ENC28J60_READ_BUF_MEM);
    while(len)
    {
        len--;
        // read data
        *data = (unsigned char)ENC28J60_BYTE_RW(0);
        data++;
    }
    *data='\0';
    ENC28J60_CSH();
...
void enc28j60WriteBuffer(unsigned int len, unsigned char* data)
{
    ENC28J60_CSL();
    // issue write command
    ENC28J60_BYTE_RW(ENC28J60_WRITE_BUF_MEM);
    
    while(len)
    {
        len--;
        ENC28J60_BYTE_RW(*data);
        data++;
    }
    ENC28J60_CSH();
}
